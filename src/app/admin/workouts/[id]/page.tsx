...
export default function WorkoutDetailPage() {
  ...
  function openEdit() {
    ...
    setEditNumber(String(workout.number))
    setEditScoreType(workout.scoreType)
    setEditLanes(String(workout.lanes))
    setEditHeatInterval(secsToField(workout.heatIntervalSecs))
    setEditTimeBetweenHeats(secsToField(workout.timeBetweenHeatsSecs))
    setEditCallTime(secsToField(workout.callTimeSecs))
    setEditWalkoutTime(secsToField(workout.walkoutTimeSecs))
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/workouts/${id}`, {
      ...
      body: JSON.stringify({
        ...
        lanes: Number(editLanes),
        heatIntervalSecs: fieldToSecs(editHeatInterval),
        timeBetweenHeatsSecs: fieldToSecs(editTimeBetweenHeats),
        callTimeSecs: fieldToSecs(editCallTime),
        walkoutTimeSecs: fieldToSecs(editWalkoutTime),
  ...
  async function generateAssignments(useCumulative = false) {
    setLoading(true)
    setMsg('')
    const res = await fetch(`/api/workouts/${id}/assignments`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function saveHeatTime(heatNum: number) {
    ...
    await fetch(`/api/workouts/${id}/heat-times`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ heatNumber: heatNum, isoTime: newHeatDate.toISOString() }),
    })
    setEditingHeatTime(null)
    await load()
  }
  ...
  async function saveAssignment(assignmentId: number) {
    await fetch(`/api/workouts/${id}/assignments`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: assignmentId, heatNumber: Number(assignEditHeat), lane: Number(assignEditLane) }),
    })
  ...
  const byHeat = workout.assignments.reduce<Record<number, Assignment[]>>((acc, a) => {
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">
            WOD {workout.number}: {workout.name}
          </h1>
          <div className="flex items-center gap-3 mt-2">
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${statusColor[workout.status] ?? 'bg-gray-700 text-gray-300'}`}>
              {workout.status}
            </span>
            ...
            {workout.startTime && (
              <span className="text-gray-400 text-sm">
                Starts {new Date(workout.startTime).toLocaleString()}
              </span>
            )}
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-red-900 hover:bg-red-800 text-red-300 text-sm font-medium rounded-lg px-4 py-2 transition-colors"
          >
            Delete
          </button>
        </div>
      ...
      {editing && (
        <div className="bg-gray-900 rounded-xl p-6 border border-orange-500/30">
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={() => setEditing(false)} className="text-gray-400 hover:text-white text-sm transition-colors">
              Cancel
            </button>
          </div>
          <form onSubmit={saveEdit} className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="number"
                value={editLanes}
                onChange={(e) => setEditLanes(e.target.value)}
                className="w-full bg-gray-800 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                required
              />
            </div>
            ...
            <div className="col-span-2">
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <div
                  onClick={() => setEditMixedHeats((v) => !v)}
                  className={`relative w-10 h-6 rounded-full transition-colors ${editMixedHeats ? 'bg-orange-500' : 'bg-gray-700'}`}
                >
                  <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${editMixedHeats ? 'translate-x-5' : 'translate-x-1'}`} />
                </div>
                <div>
                  <span className="text-sm text-white font-medium">Mixed Heats</span>
                  <p className="text-xs text-gray-500">
                    {editMixedHeats ? 'Athletes from different divisions can share a heat' : 'Each heat contains only one division'}
                  </p>
                </div>
              </label>
            </div>
            {editScoreType === 'rounds_reps' && (
              <div className="col-span-2">
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  <div
                    onClick={() => setEditTiebreakEnabled((v) => !v)}
                    className={`relative w-10 h-6 rounded-full transition-colors ${editTiebreakEnabled ? 'bg-orange-500' : 'bg-gray-700'}`}
                  >
                    <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${editTiebreakEnabled ? 'translate-x-5' : 'translate-x-1'}`} />
                  </div>
                  <div>
            ...
            <div className="col-span-2 flex gap-3">
              ...
              <button
                ...
                className="bg-gray-700 hover:bg-gray-600 text-white rounded-lg px-6 py-2.5 text-sm transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              <button
                onClick={clearScores}
                disabled={loading || workout.scores.length === 0}
              ...
              >
                ...
                {scoredCount < totalAthletes && someScored && (
                  <span className="ml-1.5 text-blue-200 text-xs">({scoredCount}/{totalAthletes})</span>
                )}
              </button>
            </div>
          </div>
                    ...
                    {editingHeatTime === heatNum ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onKeyDown={(e) => { if (e.key === 'Enter') saveHeatTime(heatNum); if (e.key === 'Escape') setEditingHeatTime(null) }}
                          autoFocus
                      ...
                      startTime && (
                        <span className="text-gray-400 text-sm flex items-center gap-2">
                          ...
                          {(() => {
                            ...
                            return (
                              <>
                                {' · '}Corral: {new Date(ms - workout.callTimeSecs * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                {' · '}Walk Out: {new Date(ms - workout.walkoutTimeSecs * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                              </>
                            )
                          })()}
                          {workout.startTime && (
                            <button
                              onClick={() => startEditHeatTime(heatNum)}
                      ...
                      return (
                        <tr key={a.id} className={`border-t border-gray-800 ${isEditingThis ? 'bg-gray-800/40' : ''}`}>
                          <td className="px-3 py-2">
                            {isEditingThis ? (
                              <input
                                type="number"
                                min="1"
                                value={assignEditLane}
                                onChange={(e) => setAssignEditLane(e.target.value)}
                                className="w-14 bg-gray-700 text-white rounded px-2 py-1 text-sm text-center focus:outline-none focus:ring-2 focus:ring-orange-500"
                              />
                            ) : (
                          ...
                          <td className="px-3 py-2">
                            {isEditingThis ? (
                              <input
                                type="number"
                                min="1"
                                value={assignEditHeat}
                                onChange={(e) => setAssignEditHeat(e.target.value)}
                                onKeyDown={(e) => { if (e.key === 'Enter') saveAssignment(a.id); if (e.key === 'Escape') setEditingAssignment(null) }}
                                className="w-14 bg-gray-700 text-white rounded px-2 py-1 text-sm text-center focus:outline-none focus:ring-2 focus:ring-orange-500"
                              />
                            ) : (
                          ...
                          <td className="px-3 py-2">
                            ...
                            {workout.scoreType === 'rounds_reps' && (
                              <div className="space-y-1">
                                <div className="flex items-center gap-1">
                                  <input
                                    type="number" min="0"
                                    value={rrInputs[a.athlete.id]?.rounds ?? ''}
                                    onChange={(e) => setRrInputs((p) => ({ ...p, [a.athlete.id]: { ...p[a.athlete.id], rounds: e.target.value } }))}
                                    placeholder="0"
                                  ...
                                  <input
                                    ...
                                    value={rrInputs[a.athlete.id]?.reps ?? ''}
                                    onChange={(e) => setRrInputs((p) => ({ ...p, [a.athlete.id]: { ...p[a.athlete.id], reps: e.target.value } }))}
                                    placeholder="0"
                                    className="w-16 bg-gray-800 text-white rounded px-2 py-1 text-sm text-center focus:outline-none focus:ring-1 focus:ring-orange-500"
                                  />
                            ...
                            {workout.scoreType !== 'time' && workout.scoreType !== 'rounds_reps' && (
                              <input
                                type="number" step="any"
                                value={weightInputs[a.athlete.id] ?? ''}
                                onChange={(e) => setWeightInputs((p) => ({ ...p, [a.athlete.id]: e.target.value }))}
                                placeholder="Score"
                            ...
                            {score ? (
                              <div>
                                ...
                                {score.tiebreakRawScore != null && (
                                  <div className="text-xs text-blue-400">TB {formatTiebreak(score.tiebreakRawScore)}</div>
                                )}
                              </div>
                            ) : '—'}
                          </td>
                          <td className="px-3 py-2 text-right">
                            {isEditingThis ? (
                              <div className="flex gap-2 justify-end">
                                <button onClick={() => saveAssignment(a.id)} className="text-xs text-green-400 hover:text-green-300 font-medium">Save</button>
